package com.blog.backend.infrastructure.seeder;
...
import com.blog.backend.identity.domain.repository.UserRoleRepository;
import com.blog.backend.interaction.domain.entity.*;
...
import com.blog.backend.interaction.domain.repository.*;
...
public class DataSeeder implements CommandLineRunner {
        ...
        public void seedData() {
                ...
                Role roleAdmin = roleRepository
                                .save(Role.builder().name("ROLE_ADMIN").status(RoleStatus.ACTIVE).build());
                Role roleUser = roleRepository.save(Role.builder().name("ROLE_USER").status(RoleStatus.ACTIVE).build());
                Role roleMod = roleRepository
                                .save(Role.builder().name("ROLE_MODERATOR").status(RoleStatus.ACTIVE).build());
                ...
                userRoleRepository.save(UserRole.builder().user(adminUser).role(roleAdmin).status(UserRoleStatus.ACTIVE)
                                .build());
                ...
                userRoleRepository.save(
                                UserRole.builder().user(modUser).role(roleMod).status(UserRoleStatus.ACTIVE).build());
                ...
                userRoleRepository.save(
                                UserRole.builder().user(userJohn).role(roleUser).status(UserRoleStatus.ACTIVE).build());
                ...
                userRoleRepository.save(UserRole.builder().user(userAlice).role(roleUser).status(UserRoleStatus.ACTIVE)
                                .build());
                ...
                userRoleRepository.save(UserRole.builder().user(userTechLead).role(roleUser)
                                .status(UserRoleStatus.ACTIVE).build());
                ...
                userRoleRepository.save(UserRole.builder().user(pendingUser).role(roleUser)
                                .status(UserRoleStatus.ACTIVE).build());
                ...
                userRoleRepository.save(UserRole.builder().user(bannedUser).role(roleUser).status(UserRoleStatus.ACTIVE)
                                .build());

                // 3.5 Categories
                Category catJava = categoryRepository.save(Category.builder().name("Java").slug("java")
                                .description("Lập trình Java và hệ sinh thái").build());
                Category catFrontend = categoryRepository.save(Category.builder().name("Frontend").slug("frontend")
                                .description("Phát triển giao diện web").build());
                Category catDatabase = categoryRepository.save(Category.builder().name("Database").slug("database")
                                .description("Cơ sở dữ liệu và tối ưu hóa").build());
                Category catArchitecture = categoryRepository.save(Category.builder().name("Architecture")
                                .slug("architecture").description("Kiến trúc phần mềm").build());
                Category catAI = categoryRepository.save(Category.builder().name("AI & Machine Learning")
                                .slug("ai-machine-learning").description("Trí tuệ nhân tạo").build());
                ...
                commentLikeRepository
                                .save(CommentLike.builder().comment(comment1).user(userTechLead).liked(true).build());
                ...
                bookmarkRepository.save(
                                Bookmark.builder().blog(blog1).user(userAlice).status(BookmarkStatus.ACTIVE).build());
                bookmarkRepository.save(
                                Bookmark.builder().blog(blog2).user(userJohn).status(BookmarkStatus.ACTIVE).build());

                // 9. Shares
                shareRepository.save(Share.builder().blog(blog1).author(userAlice).provider("FACEBOOK")
                                .status(ShareStatus.ACTIVE).build());
                shareRepository.save(Share.builder().blog(blog1).author(userTechLead).provider("LINKEDIN")
                                .status(ShareStatus.ACTIVE).build());
